...
# statistical time series models reported in Section 3.1; also includes some
# plotting functions used in Section 4

# Sourced by:
#  analysis_TimeSeriesModels.R
#  analysis_DE_ideal.R
#  analysis_DE_ESM_snapshot.R
#  analysis_DE_ESM_mean.R
...
plotTimeSeriesFirst2Weeks <- function(data_thinned, 
  ...
  par(mar=mar)
  plot.new()
  plot.window(xlim=c(1, nrow(data_thinned)), ylim=ylim)
  
  cols <- brewer.pal(n = 4, "Set1")
  
  # data
  for(i in 1:4) lines(data_thinned[, i+1], col = alpha(cols[i], alpha=1), lwd=1)
  
  # axis
  aux_seq <- round( (seq(1,  nrow(data_thinned), length=15)) / nrow(data_thinned) * 14 )
  axis(1, at = c(0, 5, 10, 14) / 14 * nrow(data_thinned), labels = c(0, 5, 10, 14))
...
# ----- Function to estimate the VAR model

EstimateVAR <- function(data, roundpoints = 4, summaries = FALSE) {
